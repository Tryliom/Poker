...
void Dealer::Update()
{
    ...
    for (int i = 0; i < NB_PLAYER; i++)
    {
    	...
    	if (player.GetPattern().GetPatternType() != PatternType::END) {
			_screen.Draw(static_cast<std::string>(player.GetPattern()), x + 3, y + TEXT_Y_OFFSET);
			x += NAME_WIDTH * 2;
    	}
    ...
    {
		_screen.Draw("Press Enter to continue or any other key to stop", _screen.GetWidth() / 2, y + 2, true);
    }
}
